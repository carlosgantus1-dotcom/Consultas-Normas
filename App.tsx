      ...
      try {
        for (const doc of sharedDocs) {
          if (doc.type === 'pdf' && !ragService.isIndexed(doc.id)) {
            // 1. Tentar carregar chunks já calculados do banco
            const chunksRes = await fetch(`/api/documents/${doc.id}/chunks`);
            const existingChunks = await chunksRes.json();
            
            if (existingChunks && existingChunks.length > 0) {
              console.log(`Carregando ${existingChunks.length} chunks do banco para: ${doc.name}`);
              ragService.loadPreCalculatedChunks(doc.id, existingChunks);
            } else {
              // 2. Se não existirem, calcular e salvar
              console.log(`Indexando novo documento via API: ${doc.name}`);
              ...
              if (content) {
                const newChunks = await ragService.indexDocument(doc.id, doc.name, content);
                if (newChunks.length > 0) {
                  ...
                  await fetch('/api/chunks', {
                    ...
                    body: JSON.stringify({
                      documentId: doc.id,
                      chunks: newChunks.map(c => ({ content: c.content, embedding: c.embedding }))
                    })
                  });
                }
              }
